...
void main()
{
	...
	if(texture(material.specular,TextCoors).r==0.0){
		//emission=texture(material.emission,TextCoors).rgb;
		emission=texture(material.emission,TextCoors+vec2(0.0,time)).rgb;
		emission=emission*(sin(time)+1.0f);
	}
